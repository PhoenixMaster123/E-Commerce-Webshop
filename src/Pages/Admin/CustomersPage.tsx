...
// Chart Components
import UserGrowthChart from "../../Components/Admin_Components/charts/UserGrowthChart";
import UserActivityHeatmap from "../../Components/Admin_Components/charts/UserActivityHeatmap";
...
const CustomersPage = () => {
    ...
    const handleAddCustomer = async (newCustomerData: Omit<Customer, "id" | "initial">) => {
        ...
        const newCustomer: Customer = {
            ...
            initial: newCustomerData.name.charAt(0).toUpperCase(),
            ...newCustomerData,
        };
        setCustomers((prevCustomers) =>
            [...prevCustomers, newCustomer].sort((a, b) => a.name.localeCompare(b.name))
        );
    ...
    const handleUpdateCustomer = async (id: string, updatedData: Partial<Customer>) => {
        ...
        setCustomers((prevCustomers) =>
            prevCustomers
                .map((customer) => (customer.id === id ? { ...customer, ...updatedData } : customer))
    ...
    const newUsersToday = 243;
    const activeUsers = 98520;
    const churnRate = 2.4;
    ...
    return (
        <div className={`flex-1 overflow-auto relative z-10 ${pageBgClass}`}>
            ...
            <AnimatePresence>
                {showAddCustomerForm && (
                    <AddCustomerForm onClose={() => setShowAddCustomerForm(false)} onAddCustomer={handleAddCustomer} isAdding={isAddingCustomer} />
                )}
            </AnimatePresence>
