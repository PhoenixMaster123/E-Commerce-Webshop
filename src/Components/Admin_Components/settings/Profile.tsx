...
const Profile = () => {
	...
	return (
		<SettingSection icon={User} title={"Profile"} theme={isDarkMode ? "dark" : "light"}>
			<div className="flex flex-col sm:flex-row items-center mb-6 space-y-4 sm:space-y-0 sm:space-x-6">
				<div className="relative w-24 h-24 sm:w-28 sm:h-28 flex-shrink-0">
					<div className={`w-full h-full rounded-full flex items-center justify-center text-5xl font-bold
                                 ${isDarkMode ? 'bg-gray-700 text-gray-400 border-2 border-gray-600' : 'bg-gray-200 text-gray-500 border-2 border-gray-300'}`}>
						JD
					</div>
				</div>
				...
				<div className="text-center sm:text-left">
					<h3 className={`text-xl font-semibold mb-1 ${isDarkMode ? 'text-white' : 'text-gray-900'}`}>John Doe</h3>
					<p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-gray-600'}`}>john.doe@example.com</p>
					<p className={`text-sm ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`}>Admin User</p>
				</div>
			</div>
